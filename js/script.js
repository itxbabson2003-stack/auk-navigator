...
map.on('click', e => { if (routeLayer) map.removeLayer(routeLayer); modernStartPoint = e.latlng; setModernStartMarker(modernStartPoint, `You: ${e.latlng.lat.toFixed(6)}, ${e.latlng.lng.toFixed(6)}`); if (destSelect.value) drawModernRoute(); });
...
function startLiveLocation() {
  ...
  liveLocationBtn.textContent = 'Stop live location'; liveLocationBtn.disabled = true; liveStatus.textContent = 'GPS: ACQUIRING...';
  navigator.geolocation.getCurrentPosition(pos => {
    const coords = { lat: pos.coords.latitude, lng: pos.coords.longitude }; modernStartPoint = coords;
    setModernStartMarker(coords, `Live: ${coords.lat.toFixed(6)}, ${coords.lng.toFixed(6)}`); map.setView(coords, 18);
    liveStatus.textContent = `GPS: LIVE ${pos.coords.accuracy.toFixed(0)}m`; if (destSelect.value) drawModernRoute();
    if (liveWatchId === null) { liveWatchId = navigator.geolocation.watchPosition(p => { const c = { lat: p.coords.latitude, lng: p.coords.longitude }; modernStartPoint = c; setModernStartMarker(c, `Live: ${c.lat.toFixed(6)}, ${c.lng.toFixed(6)}`); if (destSelect.value) drawModernRoute(); }, e => { liveStatus.textContent = `Error: ${e.message}`; }, { enableHighAccuracy: true }); }
    liveLocationBtn.disabled = false;
  }, err => { liveStatus.textContent = `Error: ${err.message}`; liveLocationBtn.disabled = false; alert(`GPS Error: ${err.message}`); }, { enableHighAccuracy: true, timeout: 10000 });
}
